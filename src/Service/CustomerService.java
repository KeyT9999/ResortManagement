...
public class CustomerService implements ICustomerService {
    ...
    public void update(Customer c) {
        ...
        while (isEditing) {
            ...
            int choice = Validation.validateInteger("Choose Your Option", errMsg, 0);
            ...
            try {
                switch (choice) {
                    case 1 -> {
                        String ID;
                        do {
                            ID = Validation.validateID("Customer ID", "ID Must Follow CUS-0000", "CUS-\\d{4}");
                            if (findByID(ID) != null) {
                    ...
                    case 2 -> {
                        String name = Validation.normalizeName(Validation.validateStringInput("Customer Full Name", errMsg));
                        selectedField.set(c, name);
                    }
                    case 3 -> {
                        LocalDate DOB = Validation.validateDateOfBirth("Customer Date Of Birth", errMsg);
                        selectedField.set(c, DOB);
                    }
                    case 4 -> {
                        String CMND = Validation.validateIDCard("Customer CMND", errMsg);
                        selectedField.set(c, CMND);
                    }
                    case 5 -> {
                        boolean isMale = Validation.validateGender("Gender (Male (M) / Female (F))", errMsg).equalsIgnoreCase("Male");
                        selectedField.set(c, isMale);
                    }
                    case 6 -> {
                        String phoneNum = Validation.validatePhoneNumber("Customer Phone Number", errMsg);
                        selectedField.set(c, phoneNum);
                    }
                    case 7 -> {
                        String email = Validation.validateEmail("Customer Email", errMsg);
                        selectedField.set(c, email);
                    }
                    case 8 -> {
                        String address = Validation.validateStringInput("Customer Address", errMsg);
                        selectedField.set(c, address);
                    }
                    case 9 -> {
                        String type = Validation.validateStringInput("Customer Type", errMsg);
    ...
    public void addCustomer() {
        do {
            try {
                ...
                if (Validation.validateStringInput("-> Do you want to save the customer data to file (Y/N)", errMsg).equalsIgnoreCase("Y")) {
                    save();
                }
            } catch (Exception e) {
                System.out.println("Error adding customer: " + e.getMessage());
            }
        } while (Validation.validateStringInput("-> Do you want to continue adding customers (Y/N)", errMsg).equalsIgnoreCase("Y"));
    }
}
